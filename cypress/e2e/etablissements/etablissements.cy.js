...
describe('Établissements de transfert – Ajout & Modification', () => {
  ...
  it('Ajouter un établissement de transfert', () => {
    ...
    // Assertion métier
    cy.messageSucces('.alert-success')
  });
  ...
  it('Modifier complètement un établissement existant', () => {
    ...
    cy.get('.page-content > :nth-child(1) > .card > .card-body')
    .should('exist')
    .within(() => {
      cy.get(':nth-child(4) > :nth-child(7) > .btn-group > .btn-sm.btn-primary')
        .click({ force: true });
    });
    ...
    cy.get('.alert-success').should('contain', 'Modification effectuée avec succès');
  });
